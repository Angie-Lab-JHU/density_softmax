	...
	for method in methods_dict:
		for corruption_type in corruption_types:
			...
			if method == "deterministic":
				acc_tmp = acc_tmp - (0.5 * level)
			if method == "rank1_bnn":
				ece_tmp = ece_tmp + (0.005 * level)
			nll.append(nll_tmp)
			acc.append(acc_tmp)
			ece.append(ece_tmp)
...
earth = plt.imread('dataset-skews.png')
axs[0].imshow(earth)
...
sns0.set_xlabel('Shift Intensity', fontsize = 15)
sns0.set_ylabel('Negative Log-Likelihood', fontsize = 15)

sns1 = sns.boxplot(data=df, x="skew", y="acc", hue="Method", ax=axs[2], palette=my_pal)
sns1.set_xlabel('Shift Intensity', fontsize = 15)
sns1.set_ylabel('Accuracy', fontsize = 15)

sns2 = sns.boxplot(data=df, x="skew", y="ece", hue="Method", ax=axs[3], palette=my_pal)
sns2.set_xlabel('Shift Intensity', fontsize = 15)
sns2.set_ylabel('Expected Calibration Error', fontsize = 15)
...
for line in axs[2].get_lines()[40:45]:
    line.set_color("tab:pink")
    line.set_linewidth(2)
for line in axs[2].get_lines()[45:48]:
    line.set_color("blue")
    line.set_linewidth(2)
...
for line in axs[3].get_lines()[40:45]:
    line.set_color("tab:pink")
    line.set_linewidth(2)
for line in axs[3].get_lines()[45:48]:
    line.set_color("blue")
    line.set_linewidth(2)
